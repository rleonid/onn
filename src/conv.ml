...
module Conv (M : Matrix_intf) = struct
  ...
  let conv_widths input ~fs ~zp ~st =
    let iaw w =
      ia "Can't fit stride %d in width %d, field size %d and padding %d"
        st w fs zp
    in
    let h1 = M.rows input in
    let dw = h1 - fs + 2 * zp in
    if dw mod st <> 0 then
      iaw h1
    else
      let h2 = dw / st + 1 in
      let w1 = M.cols input in
      ...
      unroll_mat_gen ~fs ~zp ?z ~st m
        (fun m inds col_idx ->
          Array.iteri (fun i ind ->
            M.set mat (ro + i + 1) col_idx (m ind))
            inds;
          col_idx + 1)
        1
    in
